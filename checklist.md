...
-  Check that the index page has an sleek or cinematic hero page (with some nice animation), with a block section the top 3 - 5 category.json pages as a an elegant card grid, the section below it can be the "latest post" block section with 4 - 7 latest posts as regular post blog card single row grid.

-  Check that category.json actually updates the elegant card grid and that the pages actually are containing all the set tags and are able to paginate these pages.
